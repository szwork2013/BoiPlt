...
module Protiviti.Boilerplate.Client.Controllers {
    'use strict';
    var controllerId = Modules.RoleManager.controllers.editRole;
    angular.module(Modules.RoleManager.Name).controller(controllerId,
        ['$scope', '$state', '$routeParams', '$stateParams', '$http', 'modalService', 'common', 'authService', 'breeze', 'dataservice', 'config', EditRoleController]);
    ...
    function EditRoleController($scope, $state, $routeParams, $stateParams, $http, modalService, common, authService, breeze, dataservice, config) {
       ...
       // Methods
        ...
        $scope.saveRole = function () {
            ...
            var data;
            if (roleId.length <= 0) {
                $http.post(Modules.RoleManager.BaseUrl + '/SaveChanges', newRole).then(function (response) {
                    data = response.data;
                ...
                $http.put(Modules.RoleManager.BaseUrl + '/UpdateRole', newRole).then(function (response) {
                    data = response.data;
                    showSuccessMessage(data);
                }), (function (error) {
